...
contract UniversalTokenRouter is IUniversalTokenRouter {
        ...
        unchecked {
            ...
            for (uint i = 0; i < actions.length; ++i) {
                ...
                for (uint j = 0; j < action.inputs.length; ++j) {
                    ...
                    address sender = mode == TRANSFER_FROM_ROUTER
                        ? address(this)
                        : msg.sender;
                    uint amount;
                    if (input.amountSource == AMOUNT_EXACT) {
                        ...
                        require(
                            amount <= input.amountInMax,
                            "UniversalTokenRouter: EXCESSIVE_INPUT_AMOUNT"
                        );
                    }
                ...
                if (action.data.length > 0) {
                    ...
                    (bool success, bytes memory result) = action.code.call{
                        value: value
                    }(action.data);
            ...
            for (uint i = 0; i < outputs.length; ++i) {
                ...
                require(
                    balance >= output.amountOutMin,
                    "UniversalTokenRouter: INSUFFICIENT_OUTPUT_AMOUNT"
                );
            }
    ...
    function pay(
        ...
        unchecked {
            bytes32 key = keccak256(
                abi.encodePacked(sender, recipient, eip, token, id)
            );
            require(
                s_payments[key] >= amount,
                "UniversalTokenRouter: INSUFFICIENT_ALLOWANCE"
            );
            s_payments[key] -= amount;
    ...
    function _transferToken(
        ...
        if (eip == 20) {
            ...
            require(
                sender == address(this),
                "UniversalTokenRouter: INVALID_ETH_SENDER"
            );
